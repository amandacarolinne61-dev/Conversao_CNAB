...
export function parseRetorno(conteudo) {
  ...
  const movimentos = detalhes.map((l) => {
    ...
    // Normaliza removendo a barra "/" (formato antigo da factoring) e
    // colocando em MAIÚSCULO - confirmado que a factoring às vezes manda a
    // letra da parcela em minúsculo (ex: "202600636/b"), o que quebrava o
    // casamento com o seu_numero salvo (sempre maiúsculo, ex "202600636B").
    const referenciaNormalizada = referenciaTitulo.replace(/\//g, '').toUpperCase()
