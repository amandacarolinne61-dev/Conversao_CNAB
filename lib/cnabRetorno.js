...
function parseRetorno(conteudoArquivo) {
  ...
  for (const [indice, linha] of linhas.entries()) {
    ...
    if (tipo === '0') {
      ...
      // Linha de detalhe/movimento precisa ter pelo menos 70 caracteres
      // pra existir a posição do Nosso Número (63-70). Se vier mais curta
      // ou com o campo em branco, a linha é ignorada em vez de travar o
      // upload inteiro com um valor nulo indo pro banco.
      if (linha.length < 70) {
        linhasIgnoradas.push({ numeroLinha: indice + 1, motivo: 'linha mais curta que 70 caracteres', conteudo: linha })
        continue
      }
      const mov = parseMovimento(linha)
      if (!mov.nossoNumero) {
        linhasIgnoradas.push({ numeroLinha: indice + 1, motivo: 'Nosso Número vazio na posição 63-70', conteudo: linha })
        continue
      }
      movimentos.push(mov)
    }
